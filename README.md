...
RAG обогащает генерацию документов профессиональным контекстом из учебников по управлению карьерой. Требует запущенного Ollama с моделью `nomic-embed-text`.
...
- 5–8 конкретных рекомендаций по улучшению отклика
- При наличии базы знаний — RAG-обогащение промпта профессиональным контекстом
...
| Извлечение профиля кандидата | 0 | — | JSON Mode |
| Генерация документов | 0.5 | 3000 | JSON Mode |
| Оценка соответствия (match score) | 0.3 | — | JSON Mode |
| Уточняющие вопросы (режим формы) | 0.7 | 800 | JSON Mode |

## RAG (база знаний)

Модуль `src/lib/rag.ts` обогащает генерацию документов контекстом из профессиональных учебников.
...
2. При генерации документов запрос векторизуется и сравнивается с базой через косинусное сходство — возвращаются top-K релевантных фрагментов
